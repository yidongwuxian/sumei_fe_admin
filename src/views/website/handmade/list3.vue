...
<script>
  ...
  export default {
    ...
    created(){
      let shape = this.$route.params.shape; 
      if(shape == '入口'){
        this.zid = this.$route.params.id;  
        this.sname = this.$route.params.name;  
        localStorage.id =  this.zid;  
        localStorage.sname = this.$route.params.name;
      }else{
        if(localStorage.id && localStorage.sname){
          this.zid =  localStorage.id;
    ...
    methods:{
      ...
      //跳转到新增页
      addList(){
        this.$router.push({ name:'修改手工模块', params:{zid: this.zid, name: this.sname, type:"新增" } }) 
        localStorage.id = this.zid
        localStorage.sname =  this.sname
      },
      //跳转到编辑页
      editorList(id){
        this.$router.push({ name:'修改手工模块', params:{zid: this.zid, id:id,  name: this.sname, type:"编辑" } })
        this.$store.commit('SET_handList3_Filter', this.listPageFilter);
        localStorage.id = this.zid
        localStorage.sname = this.sname
      }, 
      //搜索查询
      searchFilter(){
        ...
        //如果当前页数<总页数
        if(25 <= this.total){
           this.getList(this.zid,this.listPageFilter);
        }else{
          this.getData();    
        }
      },
      ...
      //获取数据
      getData(){
        let listFilter = this.$store.getters.handList3Filter;
        if(listFilter){
        ...
        this.getList(this.zid,this.initParams);
      },
      //end
      handleResetSearch() {
