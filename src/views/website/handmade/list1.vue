    ...
    <el-card class="filter-container" shadow="hover">
      ...
      <div class="table-container">
            ...
            <el-table ref="returnApplyTable"
              ...
              <el-table-column label="排序" prop="sort" sortable="custom" width="180" align="center">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //跳转到新增页
      addList(){
        this.$router.push({ name:'添加/修改手工模块', params:{id:this.id,sid:this.id, name: this.sname, type:"新增"} })
        localStorage.id = this.id
        localStorage.sname = this.sname
      },
      //跳转到编辑页
      editorList(id){
        this.$router.push({ name:'添加/修改手工模块', params:{id:id,sid:this.id, name: this.sname, type:"编辑"} })
        this.$store.commit('SET_handList1_Filter', this.listPageFilter);
        localStorage.id = this.id
        localStorage.sname = this.sname
      }, 
      //搜索查询
      ...
      //获取数据
      getData(){
        let listFilter = this.$store.getters.handList1Filter;
        if(listFilter){
