            ...
            <el-table ref="returnApplyTable"
              ...
              <el-table-column label="活动时间">
                <template slot-scope="scope">
                    ...
                    <span v-if="scope.row.start_time && scope.row.end_time">至</span>
...
<script>
  ...
  import {actList,delActList} from '@/api/activity';
  ...
  export default {
    ...
    methods:{
      ...
      //跳转到新增页
      addList(){
        this.$router.push({ name:'添加/修改活动', params:{} })
      },
      //跳转到编辑页
      editorList(id){
        this.$router.push({ name:'添加/修改活动', params:{id:id} })
        this.$store.commit('SET_Shop_Filter', this.listPageFilter);
      }, 
      //搜索查询
      searchFilter(){
        //dateRange
        this.dateRange ? this.listPageFilter.start_time = this.dateRange[0] : this.listPageFilter.start_time = "";
        this.dateRange ? this.listPageFilter.end_time = this.dateRange[1] : this.listPageFilter.end_time = "";
        this.$store.commit('SET_Shop_Filter', this.listPageFilter);
        //如果当前页数<总页数
        if(25 <= this.total){
      ...
      //获取数据
      getData(){
        let listFilter = this.$store.getters.shoplistFilter;
